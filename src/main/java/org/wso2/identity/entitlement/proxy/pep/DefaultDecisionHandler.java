 ...
 * 
 */
public class DefaultDecisionHandler implements DecisionHandler, PreDecisionHandler,
                                   PostDecisionHandler {
	 ...
	 * 
	 * @param request
	 *            a pepRequest object populated with the attributes
	...
	public PepResponse decide(PepRequest request) {
		...
		if ((request.getOperation() == PepRequestOperation.DECIDE) ||
			...
			azRspCtx = azHandle.queryVerbose(request.getScope(), request.getAzRequestContext());
			...
			Set<AzResourceActionAssociation> actionResourceAssociations =
			                                                              azHandle.query(request.getScope(),
			                                                                             request.getAzRequestContext(),
			                                                                             request.isQueryForAllowedResults());

			return request.getPepRequestFactory()
			              .getResponseFactory()
			              .createPepResponse(actionResourceAssociations, request,
			                                 request.isQueryForAllowedResults());
		}
		...
		return request.getPepRequestFactory().getResponseFactory()
		              .createPepResponse(azRspCtx, request, request.getOperation());
	}
