 ...
 * <p>
 * Basically, the purpose of this class is to translate the objects passed to
 * the PepRequestFactoryImpl to the associated AzAttributes used by AzApi, by
 * using the JavaObjectMappers that are designed to translate specific types of
 * Java Objects to specific sets of AzAttributes.
 * 
...
public class PepRequestImpl implements PepRequest {
	...
	private PepResponseType pepResponseType;
	private boolean resourceActionOnly;
	 ...
	 *            the actual AzRequestContext
	 * @param pepRequestFactory
	 *            the pepRequestFactory used to create this request. This handle
	 *            is used to access the <code>EnvironmentFactory</code>,
	 *            <code>SubjectFactory</code>,<code>ActionFactory</code>,
	 *            <code>ResourceFactory</code> and
	 *            <code>PepResponseFactory</code> to create the objects needed
	 *            to create the request and response.
	 ...
	 */
	protected PepRequestImpl(AzRequestContext requestContext, PepRequestFactory pepRequestFactory,
	                         PepRequestOperation operation) {
		super();
		this.azRequestContext = requestContext;
	...
	/**
	 * Calls the AzService for a decision and returns the information to the
	 * caller
	 * in the form of a <code>PepResponse</code>
	 * 
	 * @return a PepResponse from the configured {@link DecisionHandler}
	 * @throws PepException
	 *             if there was an error in processing the request. There
	 *             is some ability by the <code>PepResponseFactory</code> to
	 *             determine under
	...
	public PepResponse decide() throws PepException {
		...
		for (PreDecisionHandler preHandler : this.getPepRequestFactory().getPreDecisionHandlers()) {
			preHandler.preDecide(this);
		}

		PepResponse response = this.getPepRequestFactory().getDecisionHandler().decide(this);

		for (PostDecisionHandler postHandler : this.getPepRequestFactory()
	 ...
	 * so any changes made against it are in no way reflected in any of the
	 * objects
	 * in the pep package
	 * 
	 ...
	 * Subject request.
	 */
	public void setAccessSubject(Object subjectObject) throws PepException {
	 ...
	 * @throws PepException
	 *             if there is no <code>JavaObjectMapper</code> configured for
	 *             the object
	 * @see JavaObjectMapperImpl
	 * @see Subject
	 */
	protected Subject createAccessSubject(Object subjectObj) throws PepException {
		if (log.isTraceEnabled())
			log.trace("\n\tPepRequest.createAccessSubject() w object: " +
			          subjectObj.getClass().getSimpleName());
		SubjectFactory subjectFactory =
		                                (SubjectFactory) getPepRequestFactory().getRequestAttributesFactory(AzCategoryIdSubjectAccess.AZ_CATEGORY_ID_SUBJECT_ACCESS);
		Subject subject = subjectFactory.createObject(this);
		if (log.isTraceEnabled())
			log.trace("Created Subject object");
		return (Subject) mapJavaObject(subjectObj, subject, subjectFactory);
	}

	public void setEnvironment(Object envObject) throws PepException {
	 ...
	 * 
	 * @param envObject
	 *            object representing the Environment ex: Map
	 * @return Wrapper object around an
	 *         <code>AzEntity&lt;AzCategoryEnvironmentId&gt;</code>
	 * @throws PepException
	 *             if there is no <code>JavaObjectMapper</code> configured for
	 *             the object
	 * @see JavaObjectMapperImpl
	 * @see Environment
	 */
	protected Environment createEnvironment(Object envObject) throws PepException {
		EnvironmentFactory environmentFactory =
		                                        (EnvironmentFactory) getPepRequestFactory().getRequestAttributesFactory(AzCategoryIdEnvironment.AZ_CATEGORY_ID_ENVIRONMENT);
		Environment env = environmentFactory.createObject(this);
		return (Environment) mapJavaObject(envObject, env, environmentFactory);
	}

	public void setResourceAction(Object resourceObj, Object actionObj) throws PepException {
	 ...
	 * @return Wrapper object around an
	 *         <code>AzEntity&lt;AzCategoryResourceId&gt;</code>
	 * @throws PepException
	 *             if there is no <code>JavaObjectMapper</code> configured for
	 *             the object
	 * @see JavaObjectMapperImpl
	 * @see Environment
	 */
	protected Resource createResource(Object resourceObj) throws PepException {
		ResourceFactory resourceFactory =
		                                  (ResourceFactory) getPepRequestFactory().getRequestAttributesFactory(AzCategoryIdResource.AZ_CATEGORY_ID_RESOURCE);
		Resource resource = resourceFactory.createObject(this);
		return (Resource) mapJavaObject(resourceObj, resource, resourceFactory);
	}
	 ...
	 * 
	 * @param actionObj
	 *            object representing the Action ex: String
	 * @return Wrapper object around an
	 *         <code>AzEntity&lt;AzCategoryActionId&gt;</code>
	 * @throws PepException
	 *             if there is no <code>JavaObjectMapper</code> configured for
	 *             the object
	 * @see JavaObjectMapperImpl
	 * @see Action
	 */
	protected Action createAction(Object actionObj) throws PepException {

		RequestAttributesFactory<AzCategoryIdAction> attrActionFactory =
		                                                                 getPepRequestFactory().getRequestAttributesFactory(AzCategoryIdAction.AZ_CATEGORY_ID_ACTION);
		...
		ActionFactory actionFactory = (ActionFactory) attrActionFactory;
		Action action = actionFactory.createObject(this);
		return (Action) mapJavaObject(actionObj, action, actionFactory);
	}
	 ...
	 * @throws PepException
	 *             if there is no mapper which can map the javaObject
	 */
	protected <T extends Enum<T> & AzCategoryId> Attributes<T> mapJavaObject(Object javaObject,
		...
		log.info("\nMapping javaObject for: AzEntity<" +
		         reqAttrT.getAzEntity().getAzCategoryId().getClass().getSimpleName() +
		         "> \n\t\twhere T = AzCategoryId: " + reqAttrT.getAzEntity().getAzCategoryId());
		List<JavaObjectMapper> mappers = reqAttrFactoryT.getMappers();
		if (log.isTraceEnabled())
			log.trace("mappers.size() = " + mappers.size());
		for (int i = 0; i < mappers.size(); i++) {

			JavaObjectMapper mapper = mappers.get(i);
			if (log.isTraceEnabled())
				log.trace("\n\tLooking at mapper type: " + mapper.getClass().getName());
		...
		// What should we do?
		throw new RuntimeException("Couldn't find a converter for " + javaObject + " " + javaObject);
	}
	 ...
	 */
	public void setBulkResourceActions(List resourceObjects, List actionObjects)
	                                                                            throws PepException {
		...
		if (bulkActionObjects.size() != bulkResourceObjects.size()) {
			throw new PepException("Actions " + bulkActionObjects + " size(" +
			                       bulkActionObjects.size() + ") does not match Resources " +
			                       bulkResourceObjects + " size(" + bulkResourceObjects.size() +
			                       ")");
		}
		...
		for (int i = 0; i < bulkResourceObjects.size(); i++) {
			...
			if (log.isTraceEnabled())
				log.trace("added resource with " + "resource.getWrappedObject().getId() = " +
				          resource.getAzEntity().getId());
			Action action = createAction(bulkActionObjects.get(i));
			if (log.isTraceEnabled())
				log.trace("added action with " + "action.getWrappedObject().getId() = " +
				...
				if (azRaaResourceId == resourceId) {
					if (log.isTraceEnabled())
						log.trace("\n\tFound resource object match: \n\t\t" + obj +
						          "\n\t with input resource object: " + azRaaResourceId + "\n");
					return obj;
				} else {
					if (log.isTraceEnabled())
	 ...
	 * 
	 * @param azRaa
	 *            an AzResourceActionAssociation reference, which is used as
	 *            an index into the List of Action objects
	 * @return the action object corresponding to the
				...
				if (azRaaActionId == actionId) {
					if (log.isTraceEnabled())
						log.trace("\n\tFound action object match: \n\t\t" + obj +
						          "\n\t with input action object: " + azRaaActionId + "\n");
					return obj;
				} else {
					if (log.isTraceEnabled())
	...
	protected void addResourceActionAssociation(Resource azResource, Set<Action> actions) {
		Set<AzEntity<AzCategoryIdAction>> newSet = new HashSet<AzEntity<AzCategoryIdAction>>();
		...
		Set<AzResourceActionAssociation> setRA =
		                                         this.azRequestContext.addResourceActionAssociation(azResource.getAzEntity(),
		                                                                                            newSet);
		Iterator<AzResourceActionAssociation> itRA = setRA.iterator();
		while (itRA.hasNext()) {
			AzResourceActionAssociation azRA = itRA.next();
			if (log.isTraceEnabled())
				log.trace("Added RA w correlationId: " + azRA.getCorrelationId());
		}
	}
	 ...
	 * is determined by the value of the {@link PepRequestQueryType} that was
	 * set in the
	...
	public PepResponseType getPepResponseType() {
		return pepResponseType;
	}

	public boolean getResourceActionOnly() {
		return resourceActionOnly;
	}

	public void setPepResponseType(PepResponseType pepResponseType) {
		this.pepResponseType = pepResponseType;
	}

	public void setResourceActionOnly(boolean resourceActionOnly) {
		this.resourceActionOnly = resourceActionOnly;
	}
