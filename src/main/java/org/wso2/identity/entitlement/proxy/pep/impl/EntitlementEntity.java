...
public class EntitlementEntity<T extends Enum<T> & AzCategoryId> implements
		AzEntity<T> {
	 ...
	 * 
	 * @param t
	 *            the AzCategoryId for this AzEntity
	 * @param idEntityCounter
	 *            a provider-specific counter to aid tracing
	 */
	public EntitlementEntity(T t, int idEntityCounter) {
		...
		this.id = "AzEntity(" + t.getClass().getSimpleName() + ")-"
				+ new Integer(idEntityCounter).toString();
		if (log.isTraceEnabled())
			log.trace("\n============================================"
					+ "============================================="
					+ "\n   Constructor executed and "
					+ "created new AzEntity<" + t.getClass().getSimpleName()
					+ "> with: " + "\n\tAzCategoryId type:  " + t
	...
	/**
	 * Provider implementation to allow setting ObligationId for current
	 * AzEntity only if AzCategoryId is Obligation. Does not allow setting for
	 * other AzCategoryId values.
	 * 
	 * Note: It's a hack, but any generalization appears to be more work than
	 * reasonable. We may want to consider an enhanced AzEntity interface that
	 * allows decorating AzEntity<AzCategoryId ??? > for only specific
	 * categories. Right now there are slightly different semantics around the
	 * AzEntity as a function of AzCategoryId. There is a tradeoff between
	 * generalization and specialization to analyze. i.e. do we define separate
	 * objects for the slight differences, and how does that work around
	 * generics. It's done w the different AzDataTypes, but this one requires a
	 * new method, and input and can't be buried in the impl.
	 * 
	...
	public <U extends Enum<U> & AzDataTypeId, V> AzAttribute<T> createAzAttribute(
		...
		if (log.isTraceEnabled())
			log.trace("\n  AzEntityImpl.createAzAttribute: creating attribute: "
					...
					+ "\n\t attributeValueXacmlString: "
					+ attributeValue.toXacmlString());

		AzAttribute<T> testAzAttribute = new EntitlementAttribute<T, U, V>(
				getAzCategoryId(), issuer, attributeId, attributeValue);
		if (t.equals(AzCategoryIdObligation.AZ_CATEGORY_ID_OBLIGATION)
				|| t.equals(AzCategoryIdStatusDetail.AZ_CATEGORY_ID_STATUSDETAIL)) {
		...
		if (log.isTraceEnabled())
			log.trace("\n--------------------------------------------"
					...
					+ "\n\tto AzEntity with id:   " + this.getId()
					+ "\n\tAttribute category:    "
	...
	public AzAttributeValueAnyURI createAzAttributeValue(AzDataTypeIdAnyURI u,
			URI v) {
		AzAttributeValueAnyURI azAttrValAnyURI = new EntitlementAttributeValueAnyURI(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueBase64Binary createAzAttributeValue(
		...
		AzAttributeValueBase64Binary azAttrValBase64Binary = new EntitlementAttributeValueBase64Binary(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueBoolean createAzAttributeValue(
		...
		AzAttributeValueBoolean azAttrValBoolean = new EntitlementAttributeValueBoolean(
				v);
		if (log.isTraceEnabled())
			log.trace("\n\t.createAzAttributeValue(AzDataTypeIdBoolean u, Boolean v) "
					+ "\n\t  azAttrValBoolean.getType() = "
					+ azAttrValBoolean.getType()
					+ "\n\t  azAttrValBoolean.getValue() = "
					+ azAttrValBoolean.getValue()
					+ "\n\treturning AzAttributeValueBoolean");
		return azAttrValBoolean;
	}

	public AzAttributeValueDate createAzAttributeValue(AzDataTypeIdDate u,
			AzDataDateTime v) {
		AzAttributeValueDate azAttrValDate = new EntitlementAttributeValueDate(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueDateTime createAzAttributeValue(
		...
		AzAttributeValueDateTime azAttrValDateTime = new EntitlementAttributeValueDateTime(
				v);
		if (log.isTraceEnabled())
			log.trace("\n\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
	...
	public AzAttributeValueDayTimeDuration createAzAttributeValue(
		...
		AzAttributeValueDayTimeDuration azAttrValDayTimeDuration = new EntitlementAttributeValueDayTimeDuration(
				v);
		if (log.isTraceEnabled())
			log.trace("\n\t.createAzAttributeValue("
					+ "AzDataTypeIdDayTimeDuration u, AzDataDayTimeDuration v) "
	...
	public AzAttributeValueDnsName createAzAttributeValue(
		...
		AzAttributeValueDnsName azAttrValDnsName = new EntitlementAttributeValueDnsName(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueDouble createAzAttributeValue(AzDataTypeIdDouble u,
			Double v) {
		AzAttributeValueDouble azAttrValDouble = new EntitlementAttributeValueDouble(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueHexBinary createAzAttributeValue(
		...
		AzAttributeValueHexBinary azAttrValHexBinary = new EntitlementAttributeValueHexBinary(
				v);
		if (log.isTraceEnabled())
			log.trace("\n\t.createAzAttributeValue("
					+ "AzDataTypeIdHexBinary u, AzDataByteArray v) "
	...
	public AzAttributeValueInteger createAzAttributeValue(
		...
		AzAttributeValueInteger azAttrValInteger = new EntitlementAttributeValueInteger(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueIpAddress createAzAttributeValue(
		...
		AzAttributeValueIpAddress azAttrValIpAddress = new EntitlementAttributeValueIpAddress(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueRfc822Name createAzAttributeValue(
		...
		AzAttributeValueRfc822Name azAttrValRfc822Name = new EntitlementAttributeValueRfc822Name(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueString createAzAttributeValue(AzDataTypeIdString u,
			String v) {
		AzAttributeValueString azAttrValString = new EntitlementAttributeValueString(
				v);
		if (log.isTraceEnabled())
			log.trace("\n\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
					+ "\n\t%%%%%%%%%%%%%   using typesafe method   %%%%%%%%%%"
	...
	public AzAttributeValueTime createAzAttributeValue(AzDataTypeIdTime u,
			AzDataDateTime v) {
		AzAttributeValueTime azAttrValTime = new EntitlementAttributeValueTime(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueX500Name createAzAttributeValue(
		...
		AzAttributeValueX500Name azAttrValX500Name = new EntitlementAttributeValueX500Name(
				v);
		if (log.isTraceEnabled())
	...
	public AzAttributeValueYearMonthDuration createAzAttributeValue(
		...
		AzAttributeValueYearMonthDuration azAttrValYearMonthDuration = new EntitlementAttributeValueYearMonthDuration(
				v);
		if (log.isTraceEnabled())
			log.trace("\n\t.createAzAttributeValue("
					+ "AzDataTypeIdYearMonthDuration u, AzDataYearMonthDuration v) "
	...
	/**
	 * This looks like it should work. Only question is if it is worth the
	 * aggravation to replace all the specific signatures. It may well be,
	 * because it will be able to catch stuff that is passed that does not have
	 * supported sigs.
	 * <p>
	 * Also, and maybe more important, it will be extensible by providers to
	 * allow different impl objects.
	 * <p>
	 * Note: it appears that this method will be called in precedence over the
	 * explicit signature methods, unless the args to the explicit sig methods
	 * are totally unambiguous. i.e. if you have an unknown object coming in and
	 * you test its class using instanceof, then even if the class is correct,
	 * this generic will be called. To do instanceof you need to recast the
	 * variable when you call createAzAttribute( azDataTypeId, (Cast) object)
	 * 
	 * @param <U>
	 * @param <V>
	 * @param u
	 * @param v
	 * @return an AzAttributeValue<U,V>, where U is Xacml DataType, and V is a
	 *         corresponding Java object type
	 */
	// @SuppressWarnings("unchecked")
	public <U extends Enum<U> & AzDataTypeId, V> AzAttributeValue<U, V> createAzAttributeValue(
			U u, V v) {
		...
		if (log.isTraceEnabled())
			log.trace("\n\t&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
					...
					+ "\n\t&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
		if (u.equals(AzDataTypeIdYearMonthDuration.AZ_DATATYPE_ID_YEARMONTHDURATION)
			...
			if (log.isTraceEnabled())
				log.trace("Test 2");
			if (log.isTraceEnabled())
				log.trace("Test 2: Should not come thru here since this is "
			...
			if (log.isTraceEnabled())
				log.trace("Test 3");
			if (log.isTraceEnabled())
				log.trace("Test 3: Example of specific non-supported type combo: "
						+ "\n\t u = "
						+ u.getClass().getName()
						+ "\n\t v = "
						+ v.getClass().getName());
			...
			if (log.isTraceEnabled())
				log.trace("Unsupported Attribute value type:"
						+ "\n\t Type class u: " + u.getClass().getName()
						+ "\n\t\t Type: " + u + "\n\t Value class v: "
						+ v.getClass().getName() + "\n\t\t Value: " + v);
		}
		if (log.isTraceEnabled())
			log.trace("Exiting: createAzAttributeValue(U u, V v)");
		return azAttributeValue;
	}

	public <V extends Enum<V> & AzCategoryId> boolean addAzAttribute(
			AzAttribute<V> azAttribute) {
		boolean result = false;
		if (log.isTraceEnabled())
			log.trace("Adding pre-existing attribute of any category, V, "
		...
		if (categoryId.equals(AzCategoryIdObligation.AZ_CATEGORY_ID_OBLIGATION)
				|| categoryId
						.equals(AzCategoryIdStatusDetail.AZ_CATEGORY_ID_STATUSDETAIL)) {
			mixedAttributes.add(azAttribute);
			result = true;
		} else {
			if (log.isTraceEnabled())
				log.trace("Cannot add attribute w pre-existing category"
	...
	public <U extends Enum<U> & AzDataTypeId, V, W extends Enum<W> & AzCategoryId> boolean addAzAttribute(
			W w, String issuer, String attributeId,
			AzAttributeValue<U, V> attributeValue) {
		boolean result = false;
		if (log.isTraceEnabled())
			log.trace("  TestAzEntity.addAzAttribute: adding attribute: "
					+ "\n\t azCategoryId: " + w + "\n\t issuer: " + issuer
			...
			if (attr.getAttributeId() == attributeId) {
				if (log.isTraceEnabled())
					log.trace("\n    .getAttributeByAttribId() found attribute with: "
							...
							+ attributeId
							+ "\n\t  class:      "
							+ attr.getAzAttributeValue().getClass()
							+ "\n\t  value:      "
	...
	public AzDataDateTime createAzDataDateTime(Date date, int actualTimeZone,
			int intendedTimeZone, int nanoSeconds) {
		if (log.isTraceEnabled())
			log.trace("\n   .createAzDataDateTime(" + date + ")");
		EntitlementDataDateTime testAzDataDateTime = new EntitlementDataDateTime(
	...
	public AzDataDayTimeDuration createAzDataDayTimeDuration(
			boolean isNegativeDuration, long days, long hours, long minutes,
	...
	public AzDataYearMonthDuration createAzDataYearMonthDuration(
			boolean isNegativeDuration, long years, long months) {
		EntitlementDataYearMonthDuration testAzDataYearMonthDuration = null;
		return testAzDataYearMonthDuration;
	}

	public AzDataByteArray createAzDataByteArray(byte[] byteArray) {
		EntitlementDataByteArray azDataByteArray = new EntitlementDataByteArray(
				byteArray);
		return azDataByteArray;
	}

	/*
	 * public AzAttributeValueDateTime createAzAttributeValueDateTime(){ return
	 * new TestAzAttributeValueDateTime(); } public AzAttributeValueDateTime
	 * createAzAttributeValueDateTime( Date date, int nanoseconds, int timeZone,
	 * int defaultedTimeZone){ return new TestAzAttributeValueDateTime( date,
	 * nanoseconds, timeZone, defaultedTimeZone); }
	 * 
	 * public AzAttributeValueTime createAzAttributeValueTime(){ return new
	 * TestAzAttributeValueTime(); } public AzAttributeValueTime
	 * createAzAttributeValueTime( Date date, int nanoseconds, int timeZone, int
	 * defaultedTimeZone){ return new TestAzAttributeValueTime( date,
	 * nanoseconds, timeZone, defaultedTimeZone); }
	 * 
	 * public AzAttributeValueDate createAzAttributeValueDate(){ return new
	 * TestAzAttributeValueDate(); } public AzAttributeValueDate
	 * createAzAttributeValueDate( Date date, int timeZone, int
	 * defaultedTimeZone){ return new TestAzAttributeValueDate( date, timeZone,
	 * defaultedTimeZone); }
	 * 
	 * public AzAttributeValueString createAzAttributeValueString( String s){
	 * return new TestAzAttributeValueString(s); }
	 * 
	 * public AzAttributeValueAnyURI createAzAttributeValueAnyURI( URI uri){
	 * return new TestAzAttributeValueAnyURI(uri); }
	 * 
	 * public AzAttributeValueBoolean createAzAttributeValueBoolean( Boolean b){
	 * return new TestAzAttributeValueBoolean(b); }
	 * 
	 * public AzAttributeValueInteger createAzAttributeValueInteger( Long i){
	 * return new TestAzAttributeValueInteger(i); }
	 * 
	 * public AzAttributeValueDouble createAzAttributeValueDouble( Double d){
	 * return new TestAzAttributeValueDouble(d); }
	 * 
	 * public AzAttributeValueHexBinary createAzAttributeValueHexBinary(
	 * AzDataByteArray azBA){ return new TestAzAttributeValueHexBinary(azBA); }
	 * 
	 * public AzAttributeValueBase64Binary createAzAttributeValueBase64Binary(
	 * AzDataByteArray azBA){ return new TestAzAttributeValueBase64Binary(azBA);
	 * }
	 * 
	 * public AzAttributeValueDayTimeDuration
	 * createAzAttributeValueDayTimeDuration( AzDataDayTimeDuration azDTD){
	 * return new TestAzAttributeValueDayTimeDuration(azDTD); }
	 * 
	 * public AzAttributeValueYearMonthDuration
	 * createAzAttributeValueYearMonthDuration( AzDataYearMonthDuration azYMD){
	 * return new TestAzAttributeValueYearMonthDuration(azYMD); }
	 * 
	 * public AzAttributeValueX500Name createAzAttributeValueX500Name(
	 * X500Principal x500Principal){ return new
	 * TestAzAttributeValueX500Name(x500Principal); }
	 * 
	 * public AzAttributeValueRfc822Name createAzAttributeValueRfc822Name(
	 * String rfc822Name){ return new
	 * TestAzAttributeValueRfc822Name(rfc822Name); }
	 * 
	 * public AzAttributeValueIpAddress createAzAttributeValueIpAddress( String
	 * ipAddress){ return new TestAzAttributeValueIpAddress(ipAddress); }
	 * 
	 * public AzAttributeValueDnsName createAzAttributeValueDnsName( String
	 * dnsName){ return new TestAzAttributeValueDnsName(dnsName); }
	 */

	/*
	 * //public abstract void setAzAttribute(AzAttribute attributeTuple); public
	 * void setTestAzAttribute(AzAttribute attributeTuple) {
	 * attributes.add(attributeTuple);
	 * System.out.println("TestAzEntity.setAzAttribute: \n\t Added attribute: "
	 * + attributeTuple.getAttributeId()); return; } public boolean
	 * add(AzAttribute attributeTuple){ attributes.add(attributeTuple);
	 * System.out.println("TestAzEntity.add: \n\t Added attribute: " +
	 * attributeTuple.getAttributeId()); return true; } /* public
	 * Iterator<AzAttribute> iterator(){ return attributes.iterator(); } public
