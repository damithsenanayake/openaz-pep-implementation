...
public class EntitlementRequestContext implements AzRequestContext {
	...
	public EntitlementRequestContext() {
		...
		resourceActionAssociations = new HashSet<AzResourceActionAssociation>();
	...
	/**
	 * Create and return a new AzEntity<T> object of the specified AzCategoryId
	 * subtype T.
	 */
	public <T extends Enum<T> & AzCategoryId> AzEntity<T> createAzEntity(T t) {
		if (log.isTraceEnabled())
	...
	public <T extends Enum<T> & AzCategoryId> AzEntity<T> createNewAzEntity(T t) {
		if (log.isTraceEnabled())
			log.trace("\n\tcreateNewAzEntity(" + t.getClass().getSimpleName()
					+ ")");
		AzEntity<T> azEntity = new EntitlementEntity<T>(t, idAzEntityCounter++);
		return azEntity;
	}

	public <T extends Enum<T> & AzCategoryId> boolean addAzEntity(
			AzEntity<T> azEntity) {
	...
	/**
	 * Note: The returned Set<AzEntity<? extends AzCategoryId>> is the direct
	 * impl of the stored variables: actions, resources, etc., and probably the
	 * variable to which it returns needs to be declared as such when this
	 * method is used.
	 */
	public <T extends Enum<T> & AzCategoryId> Set<AzEntity<? extends AzCategoryId>> getAzEntitySet(
			T t) {
	...
	public <T extends Enum<T> & AzCategoryId> AzEntity<T> getAzEntity(T t) {
		...
		if (t.equals(AzCategoryIdAction.AZ_CATEGORY_ID_ACTION)) {
			...
			if (log.isTraceEnabled())
				log.trace("    getAzEntity: t = resource ");
	...
	public <T extends Enum<T> & AzCategoryId> boolean removeSubject(T t) {
	...
	/**
	 * Add the Resource and Action pair to the RequestCtx. . The Action and
	 * Resource entities MUST have an attribute with AttributeId set to the
	 * distinguished constant RequestId and ActionId. Else throw an
	 * MissingRequestIdException/MissingActionIdException.
	 * 
	 * @param azResource
	 * @param azAction
	 */
	public AzResourceActionAssociation createAndAddResourceActionAssociation(
		...
		EntitlementResourceActionAssociation testARAA = new EntitlementResourceActionAssociation(
	...
	/**
	 * Add the cross-product of the Resource and Action sets to the RequestCtx.
	 * The Action and Resource entities MUST have an attribute with AttributeId
	 * set to the distinguished constant RequestId and ActionId respectively.
	 * Else throw an MissingRequestIdException/MissingActionIdException.
	 * 
	 * @param azResourceSet
	 * @param azActionSet
	 */
	...
	/**
	 * Add the cross-product of a single AzResource and a Set of AzActions to
	 * the AzRequestContext.
	 * 
	 * @param azResource
	 * @param azActionSet
	 */
	public Set<AzResourceActionAssociation> addResourceActionAssociation(
			AzEntity<AzCategoryIdResource> azResource,
			Set<AzEntity<AzCategoryIdAction>> azActionSet) {
		Set<AzResourceActionAssociation> azRASet = new HashSet<AzResourceActionAssociation>();
		AzAttribute<AzCategoryIdResource> azAttrResourceId = azResource
				.getAttributeByAttribId(AzXacmlStrings.X_ATTR_RESOURCE_ID);
		if (!(azAttrResourceId == null)) {
			String resourceId = azAttrResourceId.getAzAttributeValue()
					.getValue().toString();
			if (log.isTraceEnabled())
				log.trace("\n\tResource being added w resource-id = "
						+ resourceId);
			if (log.isTraceEnabled())
			...
			if (log.isTraceEnabled())
				log.trace("Resource being added but has no attr w attrId = resource-id");
		}
		// resources.add(azResource);
		...
		while (it.hasNext()) {
			...
			azRASet.add(this.createAndAddResourceActionAssociation(azResource,
					azAction));
			AzAttribute<AzCategoryIdAction> azAttrActionId = azAction
					.getAttributeByAttribId(AzXacmlStrings.X_ATTR_ACTION_ID);
			if (!(azAttrActionId == null)) {
				String actionId = azAttrActionId.getAzAttributeValue()
						.getValue().toString();
				if (log.isTraceEnabled())
					log.trace("Action being added w action-id = " + actionId);
				if (log.isTraceEnabled())
					log.trace("  AzRequestContext.addResourceActionAssociation("
				...
				log.trace("Action being added but has no attr w attrId = action-id");
		}
		return azRASet;
	}

	/**
	 * Add the cross-product of a Set of AzResources and a single AzAction to
	 * the AzRequestContext.
	 * 
	 * @param azResourceSet
	 * @param azAction
	...
	/**
	 * delete the Resource and Action association to the RequestCtx, should the
	 * association exist.
	 * 
	 * @param resource
	 * @param action
	...
	/**
	 * delete the cross-product of the Resource and Action sets to the
	 * RequestCtx, should the association exist.
	 * 
	 * @param resources
	 * @param actions
	 */
	...
	/**
	 * Return the value corresponding to the distinguished constant SubjectId -
	 * from the Subject with CategoryId or null.
	 * 
	 * @param categoryId
	...
	/**
	 * Get all the values corresponding to the distinguished AttributeId
	 * resource-id found in each Resource
	 * 
	...
	/**
	 * Get all the values corresponding to the distinguished AttributeId
	 * action-id found in each Action
	 * 
	...
	/**
	 * Return the set of tuples corresponding to the resource and action pairs
	 * found in the resource context.
	 * 
	...
	/**
	 * Return the appropriate Subject component based on the CategoryId
	 * (access-subject, recipient-subject, intermediary-subject, codebase,
	 * requesting-machine) or null
	 * 
	 * @param categoryId
	 * @return
	 */
	 ...
	 * while (it.hasNext()){ azSubject = it.next(); azSubjCatId =
	 * azSubject.getAzCategoryId(); System.out.println(
	 * "TestRequestContext.getSubject: found AzSubject w cat-id: " +
	 * azSubjCatId); if (categoryId.equals(azSubject.getAzCategoryId())){
	 * System.out.println(
	 * "TestRequestContext.getSubject: found AzSubject matches requested cat-id: "
	 * + categoryId); return azSubject; } } System.out.println(
	 * "TestRequestContext.getSubject: NO AzSubject found w cat-id: " +
	 * categoryId); return azSubject; }
	 */
	...
	/**
	 * Add the Subject entity to the RequestCtx; if there is an existing Subject
	 * entity with the same categoryId, then replace it. The Subject entity MUST
	 * have an attribute with AttributeId set to the distinguished constant
	 * SubjectId, else throw an MissingSubjectIdException.
	 * 
	 * @param subject
	 */
